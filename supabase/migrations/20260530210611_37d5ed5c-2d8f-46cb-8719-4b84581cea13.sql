...
CREATE POLICY "Anyone can record interaction events"
  ...
  WITH CHECK (
    ...
    AND (surface IS NULL OR (length(surface) BETWEEN 1 AND 64 AND surface ~ '^rail:[a-z0-9-]+$'))
    AND ("position" IS NULL OR ("position" >= 0 AND "position" <= 100))
  );

CREATE INDEX IF NOT EXISTS idx_interaction_events_surface_created_at
  ON public.interaction_events (surface, created_at DESC)
