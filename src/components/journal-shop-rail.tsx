...
export function JournalShopRail({ sources, eyebrow = "From the catalog" }: { sources: Source[]; eyebrow?: string }) {
  ...
  return (
    <section className="mt-16 border-t border-ink/10 pt-12">
      <p className="text-[10px] uppercase tracking-[0.4em] text-bronze mb-8 text-center">{eyebrow}</p>
          ...
          <div key={group.handle}>
            <div className="flex items-baseline justify-between mb-4">
              ...
              <Link
                ...
                className="text-[10px] uppercase tracking-[0.3em] text-bronze hover:text-ink transition-colors"
              >
                Shop all →
              </Link>
            </div>
